...
export default function RecommendedCard({ caregiver }: { caregiver: any }) {
    ...
    return (
        <div className='flex flex-col p-2 rounded-lg bg-white transition-shadow shadow-lg hover:shadow-2xl gap-3'>
            <div className='flex flex-row-reverse flex-wrap relative'> {/* Added items-center */}

                {caregiver?.isVerified ? (
                    <div className='flex flex-col items-center justify-center ml-auto -mt-4 mr-2 absolute top-6 bg-white p-1 rounded-md'> {/* Adjusted margins */}
                        <img src={VerrifiedTickMark.src} alt="VerifiedTickMark" className='w-8 h-8' />
                        <p className='text-green-700 tracking-tight text-sm'>Vetted</p>
                    </div>
                ) : null}
                <Image
