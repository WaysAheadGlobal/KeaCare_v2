...
export default function Navbar() {
    ...
    return (
        <header className={`${pathname === "/" ? 'sticky' : 'relative'} top-0 z-50 bg-white ${pathname.includes("admin") && "hidden"}`}>
            <nav className='shadow-md'>
                <section id="navSection" className='flex flex-col lg:flex-row items-start lg:items-center gap-5 lg:gap-[3rem] justify-between px-8 pt-5 pb-[2rem] sm:pb-2 h-[7rem] overflow-hidden'>
                    ...
                    <div className='flex flex-row '>
                        ...
                        <Menu trigger='click' shadow="lg" width={200}>
                            <Menu.Target>
                                <button disabled={pathname === "/pricing" || pathname === "/caregiver/registration" || pathname === "/careseeker/registration"} className={`${!jwtToken && 'hidden'} border-2 border-teal-500 text-teal-500 bg-inherit hover:border-white hover:bg-teal-500 hover:text-white px-5 py-3 rounded-lg transition-all duration-300 cursor-pointer disabled:bg-gray-400`}>My Account</button>
                            </Menu.Target>
                            <Menu.Dropdown className='rounded-lg'>
                                <Link href={"/dashboard"}>
