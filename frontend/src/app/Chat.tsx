        ...
        if (chat) {
            ...
            try {
                if (userType === "careseeker") {
                    if (chat.toLowerCase().includes("membership")) {
                        addChat("membership");
                    } else if (chat.toLowerCase().includes("edit") && chat.toLowerCase().includes("personal") && chat.toLowerCase().includes("info")) {
                        addChat("editPersonalInfo");
                    } else if (chat.toLowerCase().includes("personal") && chat.toLowerCase().includes("info")) {
                        addChat("personalInfo");
                    } else if (chat.toLowerCase().includes("post") && chat.toLowerCase().includes("job")) {
                        addChat("postajob");
                    } else if (chat.toLowerCase().includes("search") && chat.toLowerCase().includes("caregiver")) {
                        addChat("searchcaregivers");
                    } else if (chat.toLowerCase().includes("caregiver") && chat.toLowerCase().includes("document")) {
                        addChat("caregiverdocuments");
                    } else if (chat.toLowerCase().includes("write") && chat.toLowerCase().includes("review")) {
                        addChat("writereview");
                    } else if (chat.toLowerCase().includes("view") && chat.toLowerCase().includes("payment")) {
                        addChat("viewpayments");
                    } else if (chat.toLowerCase().includes("payment")) {
                        addChat("payments");
                    } else {
                        addChat("error");
                    }
                } else if (userType === "caregiver") {
                    if (chat.toLowerCase().includes("edit") && chat.toLowerCase().includes("personal") && chat.toLowerCase().includes("info")) {
                        addChat("editPersonalInfo");
                    } else if (chat.toLowerCase().includes("see") && chat.toLowerCase().includes("personal") && chat.toLowerCase().includes("info")) {
                        addChat("seePersonalInfo");
                    } else if (chat.toLowerCase().includes("close") && chat.toLowerCase().includes("account")) {
                        addChat("closeAccount");
                    } else if (chat.toLowerCase().includes("search") && chat.toLowerCase().includes("job")) {
                        addChat("searchJobs");
                    } else if (chat.toLowerCase().includes("better") && chat.toLowerCase().includes("profile")) {
                        addChat("createBetterProfile");
                    } else if (chat.toLowerCase().includes("profile") && chat.toLowerCase().includes("not") && chat.toLowerCase().includes("approved")) {
            ...
            <section className='grid grid-rows-[6rem_1fr_6rem] w-full'>
                ...
                <div className='bg-sky-500 bg-opacity-30 flex items-center justify-evenly p-[1rem]'>
                    <input type='type' className='flex-grow h-[3rem] rounded-lg outline-none border-2 border-teal-500 hover:ring-2 hover:ring-teal-400 focus:ring-2 focus:ring-teal-400 px-3' placeholder="Type a Message." onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            appendChat(e.currentTarget.value);
