...
export default function Account() {
    ...
    useEffect(() => {
        async function getUserInfo(email?: string) {
            if (email) {
                const response = await fetch(`http://localhost:3001/api/careseeker/account?email=${email}`);
    ...
    return (
        <>
            ...
            <form className='px-[2rem] md:px-[8rem] py-[2rem] flex flex-col lg:grid lg:grid-cols-[18rem_auto] grid-rows-1 gap-10 justify-center'
                ...
                onSubmit={async (e) => {
                    e.preventDefault();
                    const response = await fetch("http://localhost:3001/api/careseeker/updateAccount", {
                        method: "PUT",
                        headers: {
