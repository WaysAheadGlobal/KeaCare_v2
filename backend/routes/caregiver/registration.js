...
const caregiverRegistationRouter = Router();
...
caregiverRegistationRouter.post("/", body('email').trim().isEmail(), async (req, res) => {
    ...
    try {
        if (!errors.isEmpty() && errors.errors[0].path === 'email') {
            ...
            if (caregiver) {
                try {
                    const { fname, lname, mobile, dob, gender, address, city, province, zipcode, device_type, status, languages, speciality, experience, comfortableWithPets, task, rate, daysAWeek, workingHrs, bio, certifications, distance, education, image: { file, name }, ref1Email, ref1Name, ref1Phone, ref1Relation, ref2Email, ref2Name, ref2Phone, ref2Relation } = req.body;
...
module.exports = { caregiverRegistationRouter, Register };