...
async function LoginOTP(req, res) {
    ...
    try {
        if (req.body.phoneNo) {
            const user = await prisma.careseekers_.findFirst({
                where: {
...
const googleLogin = (req, res) => {
    ...
    connection.query(`SELECT * FROM careseekers_ WHERE email = '${email}'`, (err, result) => {
        if (err) {
            ...
            const user = prisma.careseekers_.findFirst({
                where: {
                    email: email
                }
            });
...
module.exports = { LoginOTP, Login, googleLogin };
